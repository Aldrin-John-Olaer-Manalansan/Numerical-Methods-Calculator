...
class __MatrixStatic
{
   ...
   */
   Det(m){
      ...
      colCnt := __MatrixStatic.ColumnCount(m)
      if(!__MatrixStatic.IsSquare(m) ||  colCnt < 2)
      ...
      if(colCnt == 2){
         ...
         Loop, % colCnt
         {
            ...
            {
               laplace := __MatrixStatic.ExtractLaplace(m, [i,k]) ; extract a sub matrix by laplace
               cofactor := curVal * (((-1)**(i+k)) * __MatrixStatic.Det(laplace))
               det += cofactor
            }
         }
      }
   ...
   */
   ExtractLaplace(m, pnt){
      ...
      colCnt := __MatrixStatic.ColumnCount(m)
      rowCnt := __MatrixStatic.RowCount(m)
   ...
   */
   MultiplyScalar(A, n){   
      ...
      isColVector := (__MatrixStatic.RowCount(A) == 1)

      loop % __MatrixStatic.RowCount(A)
      {
         rowi := a_index
         Loop, % __MatrixStatic.ColumnCount(A)
         {
            if(isColVector)
   ...
   */
   Add(A, B){
      if(__MatrixStatic.ColumnCount(A) != __MatrixStatic.ColumnCount(B) 
      || __MatrixStatic.RowCount(A) != __MatrixStatic.RowCount(B))
      {
   ...
   */
	...
	*/
	Inverse(A){
		if(!__MatrixStatic.IsSquare(A))
			throw Exception("Only square matrices have an inverse! Your given matrix is not square.")
		
		size := __MatrixStatic.ColumnCount(A)
		identity := __MatrixStatic.Eye(size)
		
		inverse := __MatrixStatic.Gauss(A, identity)
		return inverse
	}
   ...
   */
   Multiply(A, B){
      mul := {}
      if(__MatrixStatic.ColumnCount(A) != __MatrixStatic.RowCount(B))
      {
      ...
      {
         rowi := A_index
         Loop, % __MatrixStatic.ColumnCount(B)
         {
            aRow := __MatrixStatic.RangeRow(A, rowi)
            bCol := __MatrixStatic.RangeCol(B, A_index)
            bColT := __MatrixStatic.Transpose(bCol)
            
            mul[rowi,A_index] := __MatrixStatic.dotP(aRow,bColT)
         }
      }
      return mul
   }
   ...
   */
   RangeCol(m, colIndex){
      ...
      Loop, % __MatrixStatic.RowCount(m)
      {
         rowi := A_index
   ...
   */
   RangeRow(m, rowIndex){
      return __MatrixStatic.Clone(m[rowIndex])
   }
   ...
   */
   Transpose(m){
      ...
      for each, row in m
      {
         ...
         if(__MatrixStatic.ColumnCount(m) == 1){
            mt[i] := row[1]
         }else{
            if(IsObject(row))
            {
	...
	IsSquare(m){
		return IsObject(m) && __MatrixStatic.ColumnCount(m) == __MatrixStatic.RowCount(m)
	}
   ...
   */
   Equals(m,m2){
      ...
      mRowCnt := __MatrixStatic.RowCount(m)
      mColCnt := __MatrixStatic.ColumnCount(m)
      
      if(mRowCnt == __MatrixStatic.RowCount(m2) 
      && mColCnt == __MatrixStatic.ColumnCount(m2)){
   ...
   */
   ToString(m){
      ...
      if(!IsObject(m))
      {
         ...
         prnt := "(" __MatrixStatic.RowCount(m) "," __MatrixStatic.ColumnCount(m) ") Matrix:`n---------`n"
         
         if(__MatrixStatic.RowCount(m) != 1)
         {
            for each, row in m
            {
               for each, item in row   
   ...
   */
	...
	* __MatrixStatic.Pivot returns an index pair of a pivot located below and to the right of i if it finds some.
	*
	*/
	Pivot(a, i="")
	{
   ...
   */
   ToRowEchelonForm(aorig, b=""){
      ...
      k := 1
      while(k < rownum){
         pivot := __MatrixStatic.Pivot(a, pivot)
         p1 := pivot[1]
         p2 := pivot[2]
	...
	*/
	RowEchelonSolve(a, b, pivot_row2col=""){
		...
		if (!pivot_row2col){
			pivot_row2col := {}
			while (pivot := __MatrixStatic.Pivot(a,pivot))
   ...
   */
   Gauss(A, B)
   {
      ...
      rowechelon := __MatrixStatic.ToRowEchelonForm(A, workerB)
      return __MatrixStatic.RowEchelonSolve(rowechelon, workerB)      
   }
...
*
*/
class __Matrix extends __MatrixStatic
{
   __new(m)
   {
      this.Prototype(m)
   ...
   */
   MultiplyScalar(m, n=0){   
      ...
      return new __Matrix(base.MultiplyScalar(m, n))
   }
   ...
   */
   Add(m, B=0){
      ...
      return new __Matrix(base.Add(m, B))
   }
   ...
   */
   Sub(B){
      ...
      return new __Matrix(base.Sub(m, B))
   }
   ...
   */
   Inverse(A=0){
      if (this != __Matrix)
         A := this
      return new __Matrix(base.Inverse(A))
   }
   ...
   */
   MultiplyRight(B){
      return new __Matrix(base.Multiply(this, B))
   }
   ...
   */
   MultiplyLeft(B){
      return new __Matrix(base.Multiply(B, this))
   }
   ...
   */
   RangeCol(m, colIndex=0){
      ...
      return new __Matrix(base.RangeCol(m, colIndex))
   }
   ...
   */
   RangeRow(m,rowIndex=0){
      ...
      return new __Matrix(base.RangeRow(m, rowIndex))
   }
   ...
   */
   Transpose(m=0){
      if (this != __Matrix)
         m := this
      return new __Matrix(base.Transpose(m))
   }
   ...
   */
   Eye(n){
     return new __Matrix(base.Eye(n))
   }
   
   Zeros(n){
      return new __Matrix(base.Zeros(n))
   }
   ...
   */
   Fill(n, fillNum){
      return  new __Matrix(base.Fill(n, fillNum))
   }
   

   Mirror2D(m){
      return new __Matrix(base.Mirror2D(m))
   }
   ...
   */
   Mirror2DByAngle(angle){
      return new __Matrix(base.Mirror2DByAngle(angle))
   }
   
   Rotate2D(angle){
      return new __Matrix(base.Rotate2D(angle))
   }
   
   Rotate3DZ(angle){
      return new __Matrix(base.Rotate3DZ(angle))
   }
   
   Rotate3DY(angle){
      return new __Matrix(base.Rotate3DY(angle))
   }
   
   Rotate3DX(angle){
      return new __Matrix(base.Rotate3DX(angle))
   }
   
   Mirror3D(axis){
      return new __Matrix(base.Mirror3D(axis))
   }
   ...
   Gauss(A, B=""){
      ...
      return new __Matrix(base.Gauss(A,B))
   }
